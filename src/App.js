...
const LIVE_FIREBASE_CONFIG = {
    ...
    projectId: "focal-campus-app",
    storageBucket: "focal-campus-app.firebasestorage.app",
...
const MOCK_PROFILES_DATA =, dislikes: ["Networking events"], location: "Tech Lab", profileImage: "https://placehold.co/100x100/171717/d4d4d4?text=BC", isDatingEnabled: true },
    { id: 'user2', name: "Alex Chen", score: 210, major: "Business Analyst", year: "4th Year", likes:, dislikes: ["Long meetings"], location: "Off-Campus Apartment", profileImage: "https://placehold.co/100x100/171717/d4d4d4?text=AC", isDatingEnabled: true },
    { id: 'user6', name: "David Kim", score: 195, major: "Data Science", year: "Grad Student", likes:, dislikes:, location: "Library Annex", isDatingEnabled: false },
    { id: 'user1', name: "Jane Doe", score: 185, major: "Electrical Eng.", year: "3rd Year", likes:, dislikes: ["Crowds"], location: "North Wing Dorms", isDatingEnabled: false },
];
...
let app_id_global = LIVE_FIREBASE_CONFIG.projectId;

const initFirebase = () => {
    // This function now uses the LIVE_FIREBASE_CONFIG object directly for connection.
    try {
        const app = initializeApp(LIVE_FIREBASE_CONFIG);
        db_instance = getFirestore(app);
        auth_instance = getAuth(app);
        console.log("Firebase initialized successfully for Live Environment.");
    } catch (e) {
        // This catch block is hit when the environment is blocked (local machine)
        console.error("Critical Firebase Connection Error:", e);
        db_instance = null;
        auth_instance = null;
    }
}
...
// --- Custom Hook to Manage Async Auth and Store ---
const useFirebaseSetup = () => {
    const [userId, setUserId] = useState(INITIAL_PROFILE.id);
    const = useState(false);
    const = useState(true);
    const = useState("Initializing...");
    
    useEffect(() => {
        initFirebase(); // Initialize Firebase instances synchronously
        ...
        if (auth_instance) {
            const unsubscribe = onAuthStateChanged(auth_instance, async (user) => {
                ...
                // Auth is now complete, user is either anonymous or signed in
                setUserId(user.uid);
                setIsDevMode(false);
                setStatusError(null);
                setIsReady(true);
            }, (error) => {
                // If authentication fails (e.g., security rule block)
                console.error("Auth Failure:", error);
                setUserId(crypto.randomUUID());
                setStatusError("SECURITY ERROR: Check Firebase Rules & Authentication.");
                setIsDevMode(true);
                setIsReady(true);
            });
            return () => unsubscribe();
        } else {
            // Local Dev Mode (No successful connection)
            setUserId(crypto.randomUUID());
            setStatusError("DEV MODE: Firebase is inactive. Data is NOT saved.");
            setIsDevMode(true);
            setIsReady(true);
        }
    },);

    return { userId, isDevMode, isReady, statusError };
};
...
// --- Gemini API Call Functions (Mocked for stability) ---
                ...
                {displayedProfiles.length > 0? (
                    displayedProfiles.map(user => (
                        <ProfileCard
                            key={user.id}
                            user={user}
                            datingMessage={datingMessage[user.id] |

| "Tap 'Generate' to craft a perfect opener."}
...
| user.rank} className={`leaderboard-row ${index < 3? 'top-rank' : ''}`}>
...
const ChatWindow = ({ targetChatId, currentUserId, targetUser, isDevMode }) => {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            // Update the last message field on the parent chat document for both users
            const chatRefUser1 = doc(db_instance, 'artifacts', app_id_global, 'users', currentUserId, 'chats', targetChatId);
            const chatRefUser2 = doc(db_instance, 'artifacts', app_id_global, 'users', targetUser.id, 'chats', targetChatId);
            ...
            await setDoc(chatRefUser1, chatData, { merge: true });
            await setDoc(chatRefUser2, {...chatData, recipientId: currentUserId, name: "Yourself" }, { merge: true });
...
| index} className={`message-row ${msg.senderId === currentUserId? 'mine' : 'theirs'}`}>
            ...
            <form className="message-input-area" onSubmit={handleSend}>
                <input
                    ...
                    placeholder={isDevMode? "Message (DEV MODE: MOCKED)" : `Message ${targetUser.name}...`}
...
const MessagesView = ({ profile, allUsers, setTargetChatId, targetChatId }) => {
    ...
    const selectedChat = targetChatId? conversations.find(c => c.id === targetChatId) : conversations;
...
| allUsers.find(u => u.id === selectedChat.recipientId)
    ...
    useEffect(() => {
        if (isDevMode) {
            ...
            if (!targetChatId && MOCK_CONVERSATIONS.length > 0) {
                 setTargetChatId(MOCK_CONVERSATIONS.id);
            }
            return;
        }
    ...
    const ConversationItem = ({ chat }) => {
        ...
        const target = allUsers.find(u => u.id === chat.recipientId) |
...
| { name: 'Unknown User' };
        ...
        return (
            ...
            >
                ...
                <span className="chat-last-time">{chat.timestamp? (chat.timestamp.toDate? chat.timestamp.toDate().toLocaleDateString(, { month: 'short', day: 'numeric' }) : new Date(chat.timestamp).toLocaleDateString(, { month: 'short', day: 'numeric' })) : ''}</span>
            </div>
        );
    };
...
export default function App() {
    const { userId, isDevMode, isReady, statusError } = useFirebaseSetup();

    const = useState(TABS.PROFILE);
    const [profile, setProfile] = useState(INITIAL_PROFILE);
    const [allUsers, setAllUsers] = useState(MOCK_PROFILES_DATA);
    const = useState(null);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(profileRef, (docSnap) => {
            ...
            // This error is caught by useFirebaseSetup and reflected in statusError
        });
...
| isDevMode) {
    ...
    const renderView = () => {
        if (!isReady) {
             return <div className="loading-screen">FOCAL Initializing...</div>;
        }
